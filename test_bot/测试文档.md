...
2. **直接调用**: 脚本直接调用SpinPet SDK，无额外封装
3. **状态共享**: 通过`bot-global.js`在脚本间共享测试数据
4. **完整流程**: 涵盖创建代币→买入→卖出的完整测试流程
...
### 执行流程
1. 运行`1-create-token.js`创建测试代币
2. 运行`2-buy-tokens.js`买入100,000个代币
3. 运行`3-sell-tokens.js`卖出50,000个代币
4. 或直接运行`run-all.js`执行完整流程

整个项目结构清晰，代码简单易懂，便于快速上手和修改扩展。